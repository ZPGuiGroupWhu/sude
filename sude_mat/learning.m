...
paramNames = {'Mode','BlockSize'};
defaults = {'dense',[]};
[mode, block_size] = internal.stats.parseArgs(paramNames, defaults, varargin{:});
...
if strcmpi(mode, 'dense')
    ...
    P = build_sparse_probability_matrix_block(X_samp, k1, get_knn, rnn, id_samp, no_dims, agg_coef, k2);
end
P = sparse((P + P'))/2;
...
function P = build_sparse_probability_matrix_block(X_samp, k1, get_knn, rnn, id_samp, no_dims, agg_coef, k2)
[N, dim] = size(X_samp);
if(k1 > 0)
    row = zeros(N*k2,1);
    col = zeros(N*k2,1);
    ...
    for i = 1:N
        ...
        row(pos) = i;
        col(pos) = idx(1:k2);
        p_val(pos) = exp(-0.5*sort_dis(1:k2).^2./max(mean(sort_dis(1:k2)).^2,realmin));
    end
    P = sparse(row,col,p_val,N,N);
else
    if(N >= 5000 && dim >= 50)
