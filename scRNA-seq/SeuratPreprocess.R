...
SeuratPreprocess <- function(seuratSCE, filename, UMAP_Dim = 2){
  ...
  seuratSCE <- RunUMAP(seuratSCE, n.components = UMAP_Dim, dims=1:50)
  write.csv(seuratSCE@reductions$pca@cell.embeddings, file=file.path(filename, "Seurat.PCA.csv"))
  write.csv(seuratSCE@reductions$pca@cell.embeddings, file=file.path(paste(filename,"PCA.csv",sep='_')))
